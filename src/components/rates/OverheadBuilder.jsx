...
// to 'flat'/'month' when omitted.
const CATEGORY_DEFS = [
  {
    key: 'Facilities & Operations',
    defaults: [{ label: 'Office Rent' }, { label: 'Utilities' }, { label: 'Office Supplies' }],
  },
  {
    key: 'Insurance',
    defaults: [
      ...
      { label: 'Workers Comp (firm policy)' },
    ],
  },
  {
    key: 'Technology & Software',
    defaults: [{ label: 'Software Licenses', costType: 'per_user' }, { label: 'IT Support' }],
  },
  {
    key: 'People & Administrative',
    defaults: [
      { label: 'Professional Development', costType: 'per_user', freq: 'year' },
      { label: 'Conferences & Memberships' },
    ],
  },
  {
    key: 'Other',
    defaults: [{ label: 'Accounting & Bookkeeping' }, { label: 'Legal Fees' }],
  },
]
...
// Flat items annualize the amount alone; Per User items scale by the firm's
// total FTE count (sum of headcount across all title stacks).
function annualize(inputAmount, freqKey, costType, fteCount) {
  const freqObj = ITEM_FREQUENCIES.find((f) => f.key === freqKey) || ITEM_FREQUENCIES[1]
  const base = (Number(inputAmount) || 0) * freqObj.multiplier
  return costType === 'per_user' ? base * (Number(fteCount) || 0) : base
}

function buildDefaultItems() {
  const items = []
  for (const cat of CATEGORY_DEFS) {
    for (const def of cat.defaults) {
      items.push({
        ...
        notes: '',
        active: true,
        cost_type: def.costType || 'flat',
        input_amount: 0,
        input_freq: def.freq || 'month',
        annual_amount: 0,
      })
    }
  }
...
// Backfills items saved before the frequency selector / cost-type toggle
// existed — an item with annual_amount (or the old flat `amount` field) but
// no input_amount/input_freq displays that figure in the amount field with
// Per Year selected; any item missing cost_type defaults to 'flat'.
function normalizeItem(item, fteCount) {
  const cost_type = item.cost_type === 'per_user' ? 'per_user' : 'flat'
  if (item.input_amount != null && item.input_freq) {
    return {
      ...item,
      cost_type,
      annual_amount: item.annual_amount ?? annualize(item.input_amount, item.input_freq, cost_type, fteCount),
    }
  }
...
function LineItemRow({ item, fteCount, onChange, onDelete }) {
  ...
  return (
    <div className="border border-[#E5E7EB] rounded px-2 py-2 space-y-1.5">
      <div className="flex items-center gap-2">
        ...
        />
        <div className="flex rounded border border-[#E5E7EB] overflow-hidden shrink-0 text-xs" role="group" aria-label="Cost type">
          <button
            type="button"
            onClick={() => onChange({ cost_type: 'flat' })}
            className={`px-2 py-1 ${!isPerUser ? 'bg-[#1E3D2F] text-white' : 'bg-white text-[#6B7280] hover:bg-[#F3F4F6]'}`}
          >
            Flat
          </button>
          <button
            type="button"
            onClick={() => onChange({ cost_type: 'per_user' })}
            className={`px-2 py-1 border-l border-[#E5E7EB] ${isPerUser ? 'bg-[#1E3D2F] text-white' : 'bg-white text-[#6B7280] hover:bg-[#F3F4F6]'}`}
          >
            Per User
          </button>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-1 pl-6">
        ...
        <input
          ...
          className="flex-1 min-w-[8rem] border border-[#E5E7EB] rounded px-2 py-1 text-xs text-[#6B7280] focus:outline-none focus:ring-1 focus:ring-[#F2903A]"
        />
      </div>
    </div>
  )
...
export default function OverheadBuilder({ stacks, onClose }) {
  ...
  // Sum of headcount across all title stacks — scales "per user" line items.
  // Updates automatically whenever RateBuilder passes a fresh `stacks` prop
  // (e.g. after a headcount edit).
  const fteCount = totalFteCount(stacks)
  ...
  async function fetchData() {
    ...
    setItems(existing && existing.length > 0 ? existing.map((it) => normalizeItem(it, fteCount)) : buildDefaultItems())
    setLoading(false)
  }

  // Recomputes annual_amount whenever the amount, frequency, or cost type
  // changes, so the stored figure is always the calculated annual total.
  function updateItem(id, patch) {
    setItems((prev) =>
      prev.map((it) => {
        if (it.id !== id) return it
        const next = { ...it, ...patch }
        if ('input_amount' in patch || 'input_freq' in patch || 'cost_type' in patch) {
          next.annual_amount = annualize(next.input_amount, next.input_freq, next.cost_type, fteCount)
        }
        return next
      }),
    )
  ...
  function addItem(category) {
    setItems((prev) => [
      ...prev,
      { id: crypto.randomUUID(), category, label: '', notes: '', active: true, cost_type: 'flat', input_amount: 0, input_freq: 'month', annual_amount: 0 },
    ])
  }
